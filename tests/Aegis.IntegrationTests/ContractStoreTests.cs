...
public sealed class ContractStoreTests(SqlServerFixture fixture) : IClassFixture<SqlServerFixture>
{
    ...
    {
        var feedId = NewFeedId();
        var store = new ContractStore(fixture.ConnectionString);
        var result = await store.SyncOneAsync(ContractParser.Parse(ContractYaml(feedId)));
        ...
        var feed = await db.QuerySingleAsync<(string Name, string TeamId, string LandingPrefix)>(
            "SELECT Name, TeamId, LandingPrefix FROM dbo.Feed WHERE Name = @feedId", new { feedId });
        Assert.Equal((feedId, "ETL", "landing/bcbs/eligibility/"), feed);

        var version = await db.QuerySingleAsync<(int Version, string SpecHash)>(
            "SELECT Version, SpecHash FROM dbo.ContractVersion WHERE FeedId = (SELECT Id FROM dbo.Feed WHERE Name = @feedId)", new { feedId });
    ...
    public async Task An_unchanged_contract_does_not_create_a_new_version()
    {
        var feedId = NewFeedId();
        var store = new ContractStore(fixture.ConnectionString);
        var spec = ContractParser.Parse(ContractYaml(feedId));
        ...
        Assert.Equal(1, await db.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM dbo.ContractVersion WHERE FeedId = (SELECT Id FROM dbo.Feed WHERE Name = @feedId)", new { feedId }));
    }
    ...
    public async Task A_changed_contract_appends_the_next_version()
    {
        ...
        var changed = ContractParser.Parse(ContractYaml(feedId).Replace("owner: ETL", "owner: Claims", StringComparison.Ordinal));
        ...
        Assert.Equal("Claims", await db.ExecuteScalarAsync<string>(
            "SELECT TeamId FROM dbo.Feed WHERE Name = @feedId", new { feedId }));
    }
    ...
    public async Task A_directory_sync_skips_invalid_files_and_reports_them()
    {
        ...
        try
        {
            await File.WriteAllTextAsync(Path.Combine(dir, "good.yaml"), ContractYaml(feedId));
            ...
            Assert.Contains(results, r => r.VersionCreated && r.Error is null);
    ...
    private static string NewFeedId() => "FEED-" + Guid.NewGuid().ToString("N")[..8];

    private static string ContractYaml(string feedId) => $$"""
        feedId: {{feedId}}
        owner: ETL
